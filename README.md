...
ExcelSearch 是一个完全在本机运行的 Excel 内容搜索程序。它读取每个工作表的 C、D 列文字并建立本地索引，让用户通过记得的内容片段找到对应的 Excel 文件、工作表和行。
...
2. 等待程序完成索引。
3. 在搜索框输入 C 列“物料名称”或 D 列中的内容片段。
4. 双击结果打开原始 Excel，或右键在文件管理器中显示文件。

多个用空格分隔的关键词采用“全部包含”匹配。例如输入 `四孔 墨西哥`，只会显示同一行 C、D 列合并内容中同时包含这两个片段的结果。
